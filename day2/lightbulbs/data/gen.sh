...
grading: custom
grader_flags: $mode $score
range: 0 $score" > "$pardir/testdata.yaml"
  echo "on_reject: break
grading: default
range: 0 2000
grader_flags: max " > "$CURGROUP_DIR/testdata.yaml"
...
_setup_dirs () {
  rm -rf secret
  mkdir -p sample/sample secret
  if [[ $USE_SCORING == 1 ]]; then
    echo "on_reject: continue
range: 0 0
accept_score: 0
grading: custom
grader_flags: lamps_small 0" > sample/testdata.yaml
    echo "on_reject: continue
range: 0 2000
accept_score: 0
grading: default
grader_flags: first_error" > sample/sample/testdata.yaml
    _update_scores
  fi
}
_setup_dirs
