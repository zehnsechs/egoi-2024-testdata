...
void solve_colors(int v,int p, vector<vi>& adj,vi& col,vi& col_count,int c,int d){
    if(col[v]==c) col_count[d]++;
    for(int w:adj[v]){
      if(w==p) continue;
      solve_colors(w,v,adj,col,col_count,c,d+1);
    }
}
...
int main(){
    ...
    for(int i=0;i<n;++i){
   	 solve_colors(0,-1,adj,col,col_count[i],i,0);
    }
    int max_val = 0;
    int max_sw = 0;
    for(int i = 0;i<n;++i){
	...
	vi v_col(n,0);
	vi v_lev(n+1,0);
	solve_colors(i,parents[i],adj,col,v_col,col[i],v_level[i]);
